...
/* FUNCTION을 호출하는 게이트를 만든다.
   게이트의 특권 레벨(DPL)에 따라 사용자 모드에서도 호출이 가능하며,
   DPL이 0이면 사용자 모드에서 직접 호출할 수 없다. 사용자 모드에서 발생한
   예외 역시 DPL==0인 게이트로 들어온다.

   TYPE은 14(인터럽트 게이트) 혹은 15(트랩 게이트)여야 한다.
   인터럽트 게이트는 진입 시 인터럽트를 비활성화하지만,
   트랩 게이트는 그대로 둔다. 자세한 내용은 [IA32-v3a] 5.12.1.2 참고. */
...
/* 인터럽트 디스크립터 테이블(IDT)의 형식은 CPU가 정해 둔 그대로이다.
   자세한 내용은 [IA32-v3a] 5.10 ~ 5.12 절을 참고. */
...
/* 각 인터럽트에 대응하는 핸들러 함수 포인터 */
static intr_handler_func *intr_handlers[INTR_CNT];

/* 디버깅을 위한 인터럽트 이름 테이블 */
static const char *intr_names[INTR_CNT];

/* 외부 인터럽트는 타이머처럼 CPU 밖의 장치가 발생시키는 신호다.
   인터럽트가 꺼진 상태에서 처리되므로 중첩되지 않고 선점도 일어나지 않는다.
   이러한 핸들러는 sleep 할 수 없지만 intr_yield_on_return()을 호출해
   인터럽트 복귀 직전에 스레드 전환을 요청할 수 있다. */
